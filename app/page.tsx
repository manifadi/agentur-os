...
import { 
  ...
  Settings, Trash2, Edit3, Users, Upload, Image as ImageIcon, Pencil, Search, LogOut, Lock, AlertTriangle 
} from 'lucide-react';
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-50 font-sans p-4">
      <div className="bg-white p-8 rounded-2xl shadow-xl w-full max-w-sm border border-gray-100">
        ...
        <div className="mt-6 text-center">
          ...
          >
            {isSignUp ? 'Zurück zum Login' : 'Noch keinen Account? Registrieren'}
          </button>
        </div>
      </div>
    </div>
...
const getStatusStyle = (status) => {
  switch (status) {
    case 'In Umsetzung': return 'bg-green-100 text-green-700 border-green-200';
    case 'Beauftragt': return 'bg-green-100 text-green-700 border-green-200';
    case 'Warten auf Kunde': return 'bg-orange-100 text-orange-700 border-orange-200';
    case 'Warten auf MA': return 'bg-orange-100 text-orange-700 border-orange-200';
    case 'Korrektur': return 'bg-purple-100 text-purple-700 border-purple-200';
    case 'Anfrage': return 'bg-gray-100 text-gray-600 border-gray-200';
    case 'Abgerechnet': return 'bg-blue-100 text-blue-700 border-blue-200';
    case 'Archiviert': return 'bg-gray-100 text-gray-400 border-gray-200';
    default: return 'bg-gray-50 text-gray-600 border-gray-100';
  }
};

const STATUS_OPTIONS = [
  'Anfrage', 'Angebot offen', 'Beauftragt', 'In Umsetzung', 
  'Korrektur', 'Warten auf Kunde', 'Warten auf MA', 'Abgerechnet', 'Archiviert'
];
...
export default function AgenturDashboard() {
  ...
  const fetchData = async () => {
    ...
    if (e) setEmployees(e);
    const { data: p } = await supabase.from('projects').select(`*, employees ( id, name, initials ), clients ( name, logo_url )`).order('created_at', { ascending: false });
    if (p) setProjects(p);
    setLoading(false);
  };
  ...
  const openClientModal = (client = null) => {
    setEditingClient(client); setClientFormName(client ? client.name : ''); setClientFormLogo(null); setClientModalOpen(true);
  };
  const handleSaveClient = async () => {
    ...
    if (editingClient) {
      const { data } = await supabase.from('clients').update({ name: clientFormName, logo_url: logoUrl }).eq('id', editingClient.id).select();
      if (data) setClients(clients.map(c => c.id === data[0].id ? data[0] : c));
    } else {
      const { data } = await supabase.from('clients').insert([{ name: clientFormName, logo_url: logoUrl }]).select();
      if (data) setClients([...clients, data[0]].sort((a,b) => a.name.localeCompare(b.name)));
    }
    setUploading(false); setClientModalOpen(false);
  };
  
  // MODIFIED DELETE WITH MODAL
  const requestDeleteClient = (id) => {
    openConfirm("Kunde löschen?", "Alle Projekte dieses Kunden werden unwiderruflich gelöscht.", async () => {
  ...
  const openEmployeeModal = (emp = null) => {
    setEditingEmployee(emp); setEmployeeFormName(emp ? emp.name : ''); setEmployeeModalOpen(true);
  };
  const handleSaveEmployee = async () => {
    if (!employeeFormName.trim()) return;
    const initials = employeeFormName.split(' ').map(n => n[0]).join('').substring(0,2).toUpperCase();
    if (editingEmployee) {
      const { data } = await supabase.from('employees').update({ name: employeeFormName, initials }).eq('id', editingEmployee.id).select();
      if (data) setEmployees(employees.map(e => e.id === data[0].id ? data[0] : e));
    } else {
      const { data } = await supabase.from('employees').insert([{ name: employeeFormName, initials }]).select();
  ...
  const requestDeleteEmployee = (id) => {
    openConfirm("Mitarbeiter löschen?", "Dieser Mitarbeiter wird aus dem System entfernt.", async () => {
  ...
  const handlePdfUpload = async (e) => {
    ...
    try {
      ...
      if (data) { setSelectedProject(data[0]); setProjects(prev => prev.map(p => p.id === data[0].id ? data[0] : p)); }
  ...
  const handleCreateProject = async () => {
    ...
    const { data } = await supabase.from('projects').insert([{
        title: newProjectData.title, job_number: newProjectData.jobNr, client_id: newProjectData.clientId,
        project_manager_id: newProjectData.pmId || null, deadline: newProjectData.deadline || null, status: 'Anfrage'
      }]).select();
  ...
  const handleUpdateProject = async () => {
    const { data } = await supabase.from('projects').update({
        title: editProjectData.title, status: editProjectData.status, deadline: editProjectData.deadline || null,
        google_doc_url: editProjectData.google_doc_url, project_manager_id: editProjectData.pmId || null
    ...
    if (data) {
      ...
      setSelectedProject(data[0]); setIsEditingProject(false);
    }
  };
  
  // MODIFIED DELETE WITH MODAL
  const requestDeleteProject = () => {
    openConfirm("Projekt löschen?", "Dieses Projekt und alle Aufgaben werden gelöscht.", async () => {
  ...
  const toggleTodo = async (todoId, currentStatus) => {
    setProjectTodos(prev => prev.map(t => t.id === todoId ? { ...t, is_done: !currentStatus } : t));
    await supabase.from('todos').update({ is_done: !currentStatus }).eq('id', todoId);
  };
  const startEditingTodo = (todo) => {
    setEditingTodoId(todo.id); setEditTodoTitle(todo.title); setEditTodoAssignee(todo.assigned_to || '');
  };
  ...
  const requestDeleteTodo = (todoId) => {
    openConfirm("Aufgabe löschen?", "Möchtest du diese Aufgabe wirklich entfernen?", async () => {
  ...
  const handleAddLog = async () => {
    ...
    const { data } = await supabase.from('project_logs').insert([{ project_id: selectedProject.id, title: newLogTitle, content: newLogContent, entry_date: new Date().toISOString() }]).select();
    if (data) { setProjectLogs([data[0], ...projectLogs]); setNewLogTitle(''); setNewLogContent(''); setIsAddingLog(false); }
  };
  const startEditingLog = (log) => { setEditingLogId(log.id); setEditLogTitle(log.title); setEditLogContent(log.content); };
  const cancelEditingLog = () => { setEditingLogId(null); setEditLogTitle(''); setEditLogContent(''); };
  const handleUpdateLog = async (logId) => {
    if (!editLogTitle.trim()) return;
    const { data } = await supabase.from('project_logs').update({ title: editLogTitle, content: editLogContent }).eq('id', logId).select();
    if (data) { setProjectLogs(prev => prev.map(l => l.id === logId ? data[0] : l)); cancelEditingLog(); }
  };
  
  // MODIFIED DELETE WITH MODAL
  const requestDeleteLog = (logId) => {
    openConfirm("Eintrag löschen?", "Dieser Logbuch-Eintrag wird unwiderruflich gelöscht.", async () => {
  ...
  const openEditModal = () => {
    setEditProjectData({ id: selectedProject.id, title: selectedProject.title, status: selectedProject.status, deadline: selectedProject.deadline || '', google_doc_url: selectedProject.google_doc_url || '', pmId: selectedProject.project_manager_id || '' });
  ...
  return (
    <div className="flex min-h-screen bg-[#F9FAFB] text-gray-900 font-sans relative">
      ...
      <aside className="w-64 bg-white border-r border-gray-200 flex flex-col sticky top-0 h-screen overflow-y-auto z-10">
        <div className="p-6 flex-1 flex flex-col h-full">
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-xs font-bold text-gray-400 uppercase tracking-wider">Kunden</h2>
            <button onClick={() => openClientModal(null)} className="text-gray-400 hover:text-gray-900 transition"><Plus size={14}/></button>
          </div>
          <nav className="space-y-1 mb-8">
            <button 
              onClick={() => { setSelectedClient(null); setSelectedProject(null); }} // <-- CHANGED: Reset Project when clicking "Alle Kunden"
              className={`w-full text-left px-3 py-2 rounded-lg text-sm font-medium transition ${!selectedClient ? 'bg-gray-100 text-gray-900' : 'text-gray-600 hover:bg-gray-50'}`}
            >
              Alle Kunden
            </button>
            {clients.map(client => (
              <div key={client.id} className="group flex items-center pr-2 rounded-lg transition hover:bg-gray-50">
                <button 
                  onClick={() => { setSelectedClient(client); setSelectedProject(null); }} // <-- CHANGED: Reset Project when clicking a Client
                  className={`flex-1 text-left px-3 py-2 text-sm font-medium flex items-center gap-3 ${selectedClient?.id === client.id ? 'bg-gray-100 text-gray-900 rounded-lg' : 'text-gray-600'}`}
                >
                   {client.logo_url ? <img src={client.logo_url} alt="" className="w-4 h-4 rounded-full object-cover"/> : <div className="w-4 h-4 rounded-full bg-gray-300 overflow-hidden flex items-center justify-center text-[6px] text-white">{client.name.substring(0,2).toUpperCase()}</div>}
                   {client.name}
                </button>
                <button onClick={() => openClientModal(client)} className="opacity-0 group-hover:opacity-100 p-1 text-gray-300 hover:text-gray-600"><Settings size={12}/></button>
              </div>
            ))}
          </nav>
          
          <div className="flex justify-between items-center mb-4 mt-4 pt-6 border-t border-gray-100">
            <h2 className="text-xs font-bold text-gray-400 uppercase tracking-wider">Team</h2>
            <button onClick={() => openEmployeeModal(null)} className="text-gray-400 hover:text-gray-900 transition"><Plus size={14}/></button>
          </div>
           <div className="space-y-2 mb-auto">
            {employees.map(emp => (
              <div key={emp.id} className="group flex justify-between items-center px-3 py-1 rounded-lg hover:bg-gray-50">
                <div className="flex items-center gap-3">
                  <div className="w-6 h-6 rounded-full bg-gray-900 flex items-center justify-center text-[10px] text-white">{emp.initials}</div>
                  <span className="text-sm text-gray-600">{emp.name}</span>
                </div>
                 <button onClick={() => openEmployeeModal(emp)} className="opacity-0 group-hover:opacity-100 p-1 text-gray-300 hover:text-gray-600"><Pencil size={12}/></button>
              </div>
            ))}
          </div>

          <div className="pt-6 border-t border-gray-100 mt-6">
            <button onClick={handleLogout} className="flex items-center gap-2 text-sm text-gray-500 hover:text-red-600 w-full px-2 py-2 rounded transition hover:bg-red-50">
        ...
        {selectedProject ? (
          <>
            <div className="flex justify-between items-center mb-6">
              <button onClick={() => setSelectedProject(null)} className="flex items-center text-sm text-gray-500 hover:text-gray-900 transition-colors">
            ...
            <div className="flex justify-between items-start mb-8">
              <div>
                ...
                <h1 className="text-3xl font-bold tracking-tight mb-2">{selectedProject.title}</h1>
              ...
              <div className="text-right">
                ...
                <div className="flex items-center justify-end gap-2">
                  ...
                  <div className="w-8 h-8 rounded-full bg-gray-900 flex items-center justify-center text-xs text-white">{selectedProject.employees?.initials || '--'}</div>
                </div>
                <div className="mt-2 text-xs text-red-500 font-medium">Deadline: {selectedProject.deadline || '-'}</div>
              </div>
            </div>
                ...
                <div className="flex-1 bg-gray-50 rounded-xl border border-dashed border-gray-200 flex flex-col items-center justify-center text-gray-400 relative overflow-hidden group">
                   {uploading ? <div className="animate-pulse text-sm">Lade Datei hoch...</div> : selectedProject.offer_pdf_url ? (
                     <>
                      ...
                      <span className="text-sm font-medium text-gray-900 mb-4">Angebot hinterlegt</span>
                      <div className="flex gap-2">
                        <a href={selectedProject.offer_pdf_url} target="_blank" rel="noreferrer" className="px-4 py-2 bg-white border border-gray-200 rounded-lg text-sm text-gray-700 shadow-sm hover:bg-gray-100">Öffnen</a>
                        <button onClick={() => pdfInputRef.current?.click()} className="px-4 py-2 bg-white border border-gray-200 rounded-lg text-sm text-gray-700 shadow-sm hover:bg-gray-100">Ändern</button>
                      </div>
                     </>
                   ) : (
                     <>
                        ...
                        <button onClick={() => pdfInputRef.current?.click()} className="text-xs text-blue-600 hover:underline">PDF hochladen</button>
                     </>
                   )}
                 ...
                 <div className="mt-4 pt-4 border-t border-gray-100">
                   {selectedProject.google_doc_url ? (
                     <a href={selectedProject.google_doc_url} target="_blank" rel="noreferrer" className="flex items-center justify-center gap-2 w-full text-blue-600 bg-blue-50 py-2 rounded-lg text-sm font-medium hover:bg-blue-100 transition">Google Doc öffnen ↗</a>
                   ) : ( <div className="text-center text-sm text-gray-400">Kein Google Doc verknüpft</div> )}
                </div>
              </div>
                  ...
                  <div className="overflow-y-auto pr-2 space-y-3 flex-1">
                      ...
                      <div key={todo.id} className="flex items-center justify-between p-3 rounded-xl hover:bg-gray-50 group transition">
                        {editingTodoId === todo.id ? (
                          <div className="flex flex-1 items-center gap-2">
                            <input autoFocus type="text" className="flex-1 bg-gray-50 rounded border-none text-sm px-2 py-1 focus:ring-1 focus:ring-blue-500" value={editTodoTitle} onChange={(e) => setEditTodoTitle(e.target.value)} />
                          ...
                          <>
                            <div className="flex items-center gap-3">
                              <button onClick={() => toggleTodo(todo.id, todo.is_done)} className={`w-5 h-5 rounded border flex items-center justify-center transition-all ${todo.is_done ? 'bg-blue-500 border-blue-500' : 'border-gray-300 hover:border-blue-400'}`}>
                            ...
                            <div className="flex items-center gap-2">
                              {todo.employees && <div className="w-6 h-6 rounded-full bg-gray-200 flex items-center justify-center text-[10px] text-gray-600 font-bold" title={todo.employees.name}>{todo.employees.initials}</div>}
                              <div className="opacity-0 group-hover:opacity-100 flex gap-1 transition-opacity">
                                ...
                                <button onClick={() => requestDeleteTodo(todo.id)} className="p-1 text-gray-300 hover:text-red-500"><Trash2 size={12}/></button>
                              </div>
                            </div>
                          </>
                        )}
                    ...
                    {isAddingTodo ? (
                      <div className="flex items-center gap-2 p-2 bg-gray-50 rounded-lg animate-in fade-in slide-in-from-top-1">
                        <input autoFocus type="text" placeholder="Aufgabe..." className="flex-1 bg-transparent border-none text-sm focus:ring-0 p-1" value={newTodoTitle} onChange={(e) => setNewTodoTitle(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleAddTodo()} />
                ...
                <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 flex-1 overflow-hidden flex flex-col">
                  ...
                  {isAddingLog && (
                    <div className="mb-4 bg-gray-50 p-3 rounded-xl border border-gray-200">
                      ...
                      <textarea placeholder="Notiz..." className="w-full bg-transparent border-none text-sm text-gray-600 resize-none focus:ring-0 p-0 h-16" value={newLogContent} onChange={(e) => setNewLogContent(e.target.value)} />
                      <div className="flex justify-end gap-2 mt-2">
                        ...
                        <button onClick={handleAddLog} className="text-xs bg-gray-900 text-white px-3 py-1 rounded-md shadow-sm">Speichern</button>
                      </div>
                    </div>
                  )}
                        ...
                        {editingLogId === log.id ? (
                           <div className="bg-gray-50 p-3 rounded-xl border border-blue-200 -ml-2">
                              ...
                              <div className="flex justify-end gap-2 mt-2">
                                ...
                                <button onClick={() => handleUpdateLog(log.id)} className="text-xs bg-gray-900 text-white px-3 py-1 rounded shadow-sm">Update</button>
                              </div>
                           </div>
                        ) : (
                           <div className="pr-4 relative">
                              ...
                              <p className="text-sm text-gray-500 mt-1 leading-relaxed whitespace-pre-wrap">{log.content}</p>
            ...
            <header className="flex justify-between items-center mb-8 gap-4">
              ...
              <div className="flex items-center gap-3">
                <div className="relative">
                  ...
                  <input type="text" placeholder="Suchen..." className="pl-9 pr-4 py-2 rounded-lg border border-gray-200 text-sm bg-white focus:ring-2 focus:ring-gray-900 focus:border-transparent outline-none w-64 transition" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
                </div>
                <button onClick={() => setIsCreatingProject(true)} className="bg-gray-900 text-white px-4 py-2 rounded-lg text-sm font-medium shadow-lg hover:bg-gray-800 transition flex items-center gap-2 whitespace-nowrap"><Plus size={16} /> Neues Projekt</button>
              </div>
            </header>
              ...
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-gray-100">
                  {filteredProjects.map(project => (
                    <tr key={project.id} onClick={() => setSelectedProject(project)} className="hover:bg-gray-50 cursor-pointer transition group">
                      ...
                      <td className="py-3 px-4 text-sm font-medium text-gray-900 flex items-center gap-2">
                        {!selectedClient && project.clients?.logo_url && <img src={project.clients.logo_url} className="w-4 h-4 rounded-full"/>}
                        {project.title}
                      </td>
                      <td className="py-3 px-4"><div className="w-6 h-6 rounded-full bg-gray-200 flex items-center justify-center text-[10px] text-gray-600 font-bold" title={project.employees?.name}>{project.employees?.initials || '-'}</div></td>
                      ...
                      <td className="py-3 px-4 text-sm text-gray-500 text-right">{project.deadline}</td>
            ...
            <div className="flex flex-col items-center text-center">
              ...
              <div className="flex gap-3 w-full">
                ...
                <button 
                  ...
                  className="flex-1 py-2.5 rounded-lg bg-red-600 text-white text-sm font-medium hover:bg-red-700 shadow-sm transition"
                >
                  Löschen
                </button>
              </div>
      ...
      {/* ... OTHER MODALS (Client, Employee, Project) remain unchanged ... */}
      {/* (Ich kürze die anderen Modals hier nicht ab, sie sind im Code oben vollständig enthalten) */}
