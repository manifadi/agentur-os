...
export default function GlobalSearch() {
    ...
    useEffect(() => {
        const timer = setTimeout(async () => {
            ...
            // Search within loaded todos (optional, might be heavy if thousands of todos, but usually ok per user view)
            // Note: projects.todos is usually populated in AppContext if we fetch it. 
            // Let's assume we search loosely.
            // Actually, tasks are nested in projects usually. Let's skip deep todo search for now or do a simple flatten if available.
            // Simplified: We skip local todo search for now to avoid complexity, or check if we have a flat list.

            // 2. REMOTE SEARCH (Logs & Deep Search)
            // We search logs via Supabase
            const { data: logs } = await supabase
                .from('project_logs')
                .select('*, projects!inner(id, title, job_number)')
                .ilike('content', `%${query}%`)
                .limit(3);
                        ...
                        <>
                            ...
                            <div className="p-2">
                                ...
                                {results.clients.map(c => (
                                    ...
                                    >
                                        <div className="w-8 h-8 rounded-full bg-orange-50 flex items-center justify-center text-xs font-bold text-orange-600 shrink-0 border border-orange-100">
                                            {c.name.substring(0, 2).toUpperCase()}
                                        </div>
                                        <div>
                                            <div className="text-sm font-semibold text-gray-900 group-hover:text-orange-600 transition">{c.name}</div>
